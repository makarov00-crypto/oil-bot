    ...
    <section class="panel">
      ...
      <div class="overview-grid">
        ...
        <article class="overview-card">
          <h3>Legacy-группа</h3>
          <p>
            Для текущей карты инструментов legacy-группа пустая. Старые модули остаются в коде только как исторический слой и запасной код,
            но не выбираются как основная стратегия.
          </p>
        </article>
        <article class="overview-card">
    ...
    <section class="panel">
      ...
      <div class="steps">
        ...
        <article class="step">
          ...
          <div>
            <h3>Подтверждаем направление</h3>
            <p>
              `RSI` и `Stochastic` должны подтверждать ту же сторону движения. Для unified-группы это локальное подтверждение, без старшего ТФ.
              Стохастик теперь допускает более раннее подтверждение, если линии уже идут в нужную сторону и остаются близко друг к другу.
            </p>
          </div>
        </article>
        <article class="step">
        ...
        <article class="step">
          ...
          <div>
            <h3>Отсекаем поздний вход</h3>
            <p>
              Если движение уже ушло слишком далеко от рабочей точки, срабатывает `late entry`, и бот не берёт хвост импульса.
              Ограничение по расстоянию до `EMA20` остаётся, но для действительно сильного импульса может быть ослаблено.
            </p>
          </div>
        </article>
        <article class="step">
          ...
          <div>
            <h3>Решаем, можно ли переворачиваться</h3>
            <p>
              В `trend` и `expansion` переворот разрешён. В `compression` сначала чаще идём в `FLAT`. В `chop` переворот запрещён.
              Если сигнал всё же не дотянул до входа, unified-контур сохраняет отдельное `HOLD`-наблюдение с причинами блокировки.
            </p>
          </div>
        </article>
      </div>
    ...
    <section class="panel">
      ...
      <div class="overview-grid">
        <article class="overview-card">
          ...
          <p>
            Даже для unified-сигнала бот проверяет доступный капитал, ГО, внутренние лимиты и допустимый риск на одну сделку.
            Сильный сигнал может быть уменьшен по размеру, если по риску нельзя брать штатный объём.
          </p>
        </article>
        <article class="overview-card">
          <h3>Повторный вход</h3>
          <p>
            После слабого выхода стратегия не должна мгновенно входить снова в ту же сторону. Для этого используются cooldown и требование нового экстремума цены.
          </p>
        </article>
        <article class="overview-card">
        ...
        <article class="overview-card">
          ...
          <p>
            `HOLD` теперь тоже фиксируется как отдельное наблюдение: видно, где не хватило объёма, где сработал `late entry`,
            где рынок признан пилой, а где локальный разворот не подтвердился по направлению.
          </p>
        </article>
      </div>
    </section>
