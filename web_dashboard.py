...
INSTRUMENT_DISPLAY_NAMES: dict[str, str] = {
    ...
    "BMQ6": "BR-8.26 Нефть Brent",
    "BMU6": "BR-9.26 Нефть Brent",
    "BRU6": "BR-9.26 Нефть Brent",
    "BRV6": "BR-10.26 Нефть Brent",
...
STRATEGY_DOCS: dict[str, dict[str, str]] = {
    ...
    "reversal_1h": {
        ...
        "when": "Сопровождает только ранее открытые позиции до закрытия; новые входы выполняются через ao_chaikin_1h.",
    },
}
...
def build_strategy_docs_rows() -> tuple[str, str, list[str], list[str]]:
    cards: list[str] = []
    rows: list[str] = []
    ...
    for symbol in documentation_symbols:
        ...
        if "ao_chaikin_1h" in get_primary_strategies(symbol):
            mode = "AO / Чайкин 1ч"
...
<body>
  ...
  <main class="wrap">
    <section class="panel hero">
      ...
      <p>
        Все новые входы текущих инструментов выполняются по `ao_chaikin_1h` с 50% базового риска на сделку.
        Ранее открытые позиции `reversal_1h` сохраняют прежнее сопровождение до закрытия.
      </p>
    </section>
    <section class="panel">
      ...
      <div class="overview-grid">
        <article class="overview-card">
          <h3>AO / Чайкин 1ч</h3>
          <p>Основная логика повторяет теневую версию 3: `AO(5,34)` по средней цене high/low,
             Чайкин `EMA(5) − EMA(20)` накопленного денежного потока, `ATR(14)`.</p>
        </article>
        <article class="overview-card">
          <h3>Переход открытых позиций</h3>
          <p>Стратегия выбирается по идентификатору входа. Позиции `reversal_1h` доводятся по старым правилам.
             После закрытия следующий вход возможен только по новой стратегии.</p>
        </article>
        <article class="overview-card">
          <h3>Исторические стратегии</h3>
          <p>`reversal_15m` и остальные старые модули доступны в истории. Для новых входов они не выбираются.</p>
        </article>
      </div>
    </section>
    <section class="panel">
      <h2>Состав часовой группы</h2>
      <div class="overview-grid">
        <article class="overview-card">
        ...
        <article class="overview-card">
          <h3>Прочие инструменты в справочнике</h3>
          <p>{legacy_display}</p>
        </article>
      </div>
    </section>
    <section class="panel">
      <h2>Как работает `ao_chaikin_1h`</h2>
      <div class="steps">
        <article class="step">
          <div class="step__num">01</div>
          <div><h3>Вход на закрытых часовых свечах</h3>
            <p>Для LONG нужны свеча пересечения AO вверх через ноль и следующая усиливающаяся положительная свеча.
               Для SHORT условие зеркальное. Сила `abs(AO) / ATR` должна быть не ниже `0,60`.</p>
          </div>
        </article>
        <article class="step">
          <div class="step__num">02</div>
          <div><h3>Оценка потока Чайкина</h3>
            <p>Чайкин отмечает подтверждение, противоречие или нейтральный поток. Это оценка качества сигнала,
               которая сама по себе не блокирует вход.</p>
          </div>
        </article>
        <article class="step">
          <div class="step__num">03</div>
          <div><h3>Удержание и выход</h3>
            <p>Выходим, когда AO достигает нуля или пересекает его против позиции. Другой выход требует одновременно
               трёх последовательных ослаблений AO, остатка импульса не более 70% пика с момента входа и закрытия
               цены против позиции относительно предыдущей свечи.</p>
          </div>
        </article>
      </div>
    </section>
    <section class="panel">
      ...
      <div class="overview-grid">
        <article class="overview-card">
          <h3>50% базового риска</h3>
          <p>Новые сделки получают половину базового допустимого убытка на сделку. Число контрактов зависит от
             расстояния до стопа, целых лотов, доступного капитала, ГО и общих лимитов риска.
             Допускается один лот сверх половинного бюджета, если позволяют брокер, ГО и совокупный риск портфеля.</p>
        </article>
        <article class="overview-card">
          <h3>Защита позиции</h3>
          <p>Аварийная защита сохраняется. Программный выход новой стратегии подтверждается закрытой часовой свечой.
             Для ранее открытых `reversal_1h` продолжают действовать прежние выходы и защитные правила.</p>
        </article>
      </div>
    </section>
    <section class="panel">
...
def load_shadow_strategy_workspace() -> dict[str, Any]:
    generated_at = datetime.now(timezone.utc)
    ...
    exit_analytics = build_shadow_exit_analytics(shadow_records)
    symbols = {
        str(row.get("symbol") or "").upper()
        for row in list(comparison.get("by_symbol") or [])
        if str(row.get("symbol") or "").strip()
    }
    for row in list(strategy.get("open_positions") or []) + list(strategy.get("closed_trades") or []):
        symbol = str(row.get("symbol") or "").upper()
        if symbol:
            symbols.add(symbol)
    catalog = {
            ...
            open_positions.append(
                {
                    ...
                    "income_rub": pos.get("income_rub") if pos.get("income_rub") is not None else pos.get("expected_yield_rub"),
                    "variation_margin_source": pos.get("variation_margin_source") or "legacy_snapshot",
...
<head>
  ...
  <style>
    ...
    .portfolio-secondary-value strong {
      color: #dcecff;
      font-size: 15px;
    }
    .portfolio-label {
...
<body>
  ...
  <div class="wrap">
    ...
    <section class="panel" style="margin-bottom:16px;">
      ...
      <div class="muted" style="margin-bottom:12px;" id="portfolioMeaning">
        Счёт брокера и три показателя текущего торгового дня.
      </div>
      <div class="portfolio-layout">
        ...
        <div class="portfolio-group">
          <h3>Сегодня</h3>
          <p>Три независимых показателя из данных брокера.</p>
          <div class="portfolio-metrics">
  ...
  <script>
    ...
    function signalScopeBadge(state) {
      ...
      if (strategy === 'ao_chaikin_1h') {
        return `<span class="badge hold">AO / ЧАЙКИН 1Ч</span>`;
      }
      if (strategy === 'reversal_1h') {
    ...
    function strategyBadge(row) {
      ...
      if (strategy === 'ao_chaikin_1h') {
        return '<span class="badge long">AO / ЧАЙКИН 1Ч</span>';
      }
      if (strategy === 'reversal_1h') {
